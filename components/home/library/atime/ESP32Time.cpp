...
*/
void ESP32Time::getDateTime(char *ret, bool mode){
	struct tm timeinfo = getTimeStruct();
	if (mode)
	{
		strftime(ret, 50, "%A, %B %d %Y %H:%M:%S", &timeinfo);
	}
	else
	{
		strftime(ret, 50, "%a, %b %d %Y %H:%M:%S", &timeinfo);
...
/*!
    ...
    @param  mode
			...
			false = Short date format
*/
void ESP32Time::getTimeDate(char *ret, bool mode){
	struct tm timeinfo = getTimeStruct();
	if (mode)
	{
		strftime(ret, 50, "%H:%M:%S %A, %B %d %Y", &timeinfo);
	}
	else
	{
		strftime(ret, 50, "%H:%M:%S %a, %b %d %Y", &timeinfo);
...
*/
void ESP32Time::getTime(char *ret){
	struct tm timeinfo = getTimeStruct();
	strftime(ret, 50, "%H:%M:%S", &timeinfo);
}
...
*/
void ESP32Time::getTime(char *ret, uint8_t len, char * format){
	struct tm timeinfo = getTimeStruct();
	strftime(ret, len, format, &timeinfo);
}
...
/*!
    ...
    @param  mode
			...
			false = Short date format
*/
void ESP32Time::getDate(char *ret, bool mode){
	struct tm timeinfo = getTimeStruct();
	if (mode)
	{
		strftime(ret, 50, "%A, %B %d %Y", &timeinfo);
	}
	else
	{
		strftime(ret, 50, "%a, %b %d %Y", &timeinfo);
...
/*!
	...
	@param	lowercase
			...
			false = uppercase
*/
void ESP32Time::getAmPm(char *ret, bool lowercase){
	struct tm timeinfo = getTimeStruct();
